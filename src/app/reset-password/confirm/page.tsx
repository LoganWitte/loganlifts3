...
const Page = () => {
    ...
    // Marks presence of data from searchParams
    // Note that page will work normally unless 'tokenPresent' is false
    // Otherwise, missing data will simply not be displayed
    const namePresent = name !== null && name.length > 0;
    const emailPresent = email !== null && email.length > 0;
    ...
    return tokenPresent ? (
        <div className="flex flex-col p-4 sm:m-4 bg-slate-200 sm:border-t border-b border-l border-r border-black text-black min-w-full sm:min-w-160">
            ...
            {namePresent && <div className="flex flex-row justify-center text-lg mx-4 mb-2">
                <span className="font-bold mr-1">Name:</span>
                {name}
            </div>}

            {emailPresent && <div className="flex flex-row justify-center text-lg mx-4 mb-2">
        ...
        <div className="flex flex-col p-4 sm:m-4 bg-slate-200 sm:border-t border-b border-l border-r border-black text-black min-w-full sm:min-w-160">
            ...
            <div className="flex flex-row justify-center text-lg mx-4 mb-2">
                <Link
                    href={`/reset-password/request${emailPresent ? "?email=" + email : ""}`}
