...
const Page = () => {
    ...
    async function handleSubmit() {
        ...
        // Signs in using '/verify-email/request/route.ts'
        // This route always returns "ok" if it is able to respond
        const result = await fetch('/api/verify-email/request', {
            method: 'POST',
        ...
        if (result.ok) {
            ...
            setSubmitResponse("Error: " + (data.error ?? "Server failed to respond. Confirm internet connection or try again later."));
            setResponseIsError(true);
        }
    ...
    return (
        <div className="flex flex-col p-4 sm:m-4 bg-slate-200 sm:border-t border-b sm:border-l sm:border-r border-black text-black min-w-full sm:min-w-160">
            ...
            <div className="flex flex-row justify-center text-lg mx-4 mb-2">
                Enter your email address below and we&apos;ll send you a link to verify your email.
            </div>
            <form
